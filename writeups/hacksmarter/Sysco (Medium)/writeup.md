...
With Kerbrute it's possible to check that list of names against valid users in Active Directory. The naming format of `Firstname.Lastname@sysco.local` is identified and with that, the attacker will create a users.txt.
![[kerbrute.png]]
...
```
The attacker will confirm compromise with NetExec.
![[jackpwn3d.png]]

Additionally with this user pwn3d,  The attacker will utilize bloodhound-ce-python to identify the Active Directory configuration and additional attack vectors.
![[bloodhoundce.png]]
...
Utilizing hashcat this hash was trivial to crack. Revealing the password of `Chocolate1`.
![[crackrouterhash.png]]
![[chocolate1.png]]

Spraying the password adds `Lainey.Moore` to the pwn3d list. This user gets marked as Owned in BloodHound. Access to their mail account did not seem possible.
![[laineypwn3d.png]]
...
Since the scope indicate AV is on the box, the attacker will perform commands remotely to minimize detection. This is done with BloodyAD to take ownership over the `Default Domain Policy`.
![[escpath.png]]

![[defaultdomainpolicypwn3d.png]]
![[defaultdomainsetowner.png]]
...
The attacker set themselves as the owner and added genericAll to themselves. Once done they utilized pygpoabuse to create a scheduled task on the machine to grant themselves local administrator.
![[pwn3dbox.png]]
